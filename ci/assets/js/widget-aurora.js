(function (window, document) {
      ...
      options.predictionResponse.predictions.sort((a, b) => (a.prediction > b.prediction ? -1 : 1)).forEach(
        (prediction, index) => {
          if (index < 3) {
			...
			const code = Number(prediction.sdg.code);
			const nameEs = SDG_ES[code] || prediction.sdg.name; // fallback si falta
			textContainer.innerHTML ='SDG ' + code + ': ' + nameEs + ' (' + (prediction.prediction * 100).toFixed(0) + '%)';
            li.appendChild(link);
  ...
  const getOrCreateLegendList = (chart, id) => {
    ...
    if (!listContainer) {
      ...
      title.textContent = 'Top 3 Clasificaciones SDG (Sustainable Development Goals)'
          ...
          .then((data) => {
            ...
            const myChart = new Chart(ctx, {
              ...
              data: {
				...
				labels: [
				  'ODS 1: Eliminación de la pobreza',
				  'ODS 2: Cero hambre',
